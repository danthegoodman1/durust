...
**Baseline.** Unless a row explicitly names a later plan item, its status, test name, and source line are as of commit
...
| 27 | Change markers await unavailable history through bounded replay gates; no bulk reload or event-zero restart | `tests/replay_core.rs::recorded_version_marker_beyond_loaded_window_streams_in_order`, `::budgeted_recovery_streams_markers_without_restarting` | `worker.test.ts` — `streams thousands of markers without a replay restart`, `does not expose chunk boundaries as catchable errors` | **Both**, updated by 0019; marker event format and repeated-call sequence semantics remain unchanged |
...
| Deterministic execution disposal | **Landed** | `HotWorkflowExecution.dispose(reason)` handles failed tasks, superseded executions, conflicting facts, disabled caches and eviction. 0019 removes the obsolete reserve-exhaustion disposal because markers now suspend on the normal replay gate. Pinned by ledger row 12. |
