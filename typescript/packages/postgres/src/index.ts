...
import { historyEventType } from "@durust/core";
import { decodePayload, encodePayload, type PayloadRef } from "@durust/core";
import type {
...
interface PersistedProviderState {
  ...
  readonly nextActivityClaimToken: number;
  readonly nextSignalSequence: number;
  readonly workflowIds: readonly PersistedWorkflowId[];
  readonly workflows: readonly PersistedWorkflow[];
  readonly activities: readonly PersistedActivity[];
  readonly activityMaps: readonly PersistedActivityMap[];
  readonly childWorkflowMaps: readonly PersistedChildWorkflowMap[];
  readonly waits: readonly PersistedWait[];
  readonly signals: readonly PersistedSignal[];
}

interface PersistedWorkflowId {
  readonly key: string;
...
interface PersistedWorkflow {
  ...
  readonly readyReason: WorkflowTaskReason | null;
  readonly claim: WorkflowTaskClaim | null;
  readonly queryProjection: PayloadRef | null;
...
interface PersistedActivity {
  ...
  readonly runId: string;
  readonly task: ActivityTask;
  readonly claim: ActivityTaskClaim | null;
  readonly terminalEventId: EventId | null;
}

interface PersistedActivityMap {
...
interface PersistedChildWorkflowMap {
  readonly commandKey: string;
  readonly namespace: string;
  readonly runId: string;
  readonly task: ChildWorkflowMapTask;
...
export class PostgresBackend implements DurableBackend {
  ...
  constructor(options: PostgresBackendOptions = {}) {
    this.#tableName = sqlIdentifier(options.tableName ?? "durust_ts_provider_state");
    if (options.pool) {
  ...
  async #initialize(): Promise<void> {
    ...
    `);
    await this.#pool.query(
      `insert into ${this.#tableName}(id, state)
       values (1, $1)
       on conflict (id) do nothing`,
      [stringifyJson(emptyPersistedProviderState())]
    );
  }
  ...
  async #withState<T>(fn: () => T): Promise<T> {
    await this.#ensureReady();
    const client = await this.#pool.connect();
    try {
      await client.query("begin");
      this.#loadPersistedState(await this.#readPersistedState(client));
      const result = fn();
      const state = stringifyJson(this.#snapshotPersistedState());
      await client.query(`update ${this.#tableName} set state = $1 where id = 1`, [state]);
  ...
  async #readPersistedState(client: PoolClient): Promise<PersistedProviderState> {
    const result = await client.query<{ readonly state: string }>(
      `select state from ${this.#tableName} where id = 1 for update`
    );
    const row = result.rows[0];
    if (!row) {
      const empty = emptyPersistedProviderState();
      await client.query(`insert into ${this.#tableName}(id, state) values (1, $1)`, [
        stringifyJson(empty)
      ]);
      return empty;
    }
    return parseJson<PersistedProviderState>(row.state);
  }

  #loadPersistedState(persisted: PersistedProviderState): void {
    ...
    this.#nextRun = persisted.nextRun;
    this.#nextClaimToken = persisted.nextClaimToken;
    this.#nextActivityClaimToken = persisted.nextActivityClaimToken;
    this.#nextSignalSequence = persisted.nextSignalSequence;

    for (const workflow of persisted.workflows) {
      const state: WorkflowState = {
        namespace: workflow.namespace,
        workflowId: workflow.workflowId,
        workflowType: workflow.workflowType,
        taskQueue: workflow.taskQueue,
        runId: runId(workflow.runId),
        history: [...workflow.history],
        readyReason: workflow.readyReason,
        claim: workflow.claim,
        queryProjection: workflow.queryProjection,
        terminal: workflow.terminal,
        parent: workflow.parent
      };
      this.#workflowsByRun.set(String(state.runId), state);
    }
    for (const id of persisted.workflowIds) {
      const workflow = this.#workflowsByRun.get(id.runId);
      if (workflow) {
        this.#workflowsById.set(id.key, workflow);
      }
    }
    ...
    for (const activity of persisted.activities) {
      const workflow = this.#workflowsByRun.get(activity.runId);
      if (!workflow) {
        continue;
      }
      this.#activitiesById.set(activity.activityId, {
        namespace: activity.namespace,
        workflow,
        task: activity.task,
        claim: activity.claim,
        terminalEventId: activity.terminalEventId
      });
    }
    for (const map of persisted.activityMaps) {
      const workflow = this.#workflowsByRun.get(map.runId);
      if (!workflow) {
        continue;
      }
      this.#activityMapsByCommand.set(map.commandKey, {
        namespace: map.namespace,
        workflow,
        task: map.task,
        inputs: [...map.inputs],
        results: [...map.results],
        inFlight: new Set(map.inFlight),
        nextOrdinal: map.nextOrdinal,
        terminal: map.terminal
      });
    }
    for (const map of persisted.childWorkflowMaps) {
      const workflow = this.#workflowsByRun.get(map.runId);
      if (!workflow) {
        continue;
      }
      this.#childWorkflowMapsByCommand.set(map.commandKey, {
        namespace: map.namespace,
        workflow,
        task: map.task,
        inputs: [...map.inputs],
    ...
    for (const wait of persisted.waits) {
      this.#waitsById.set(wait.waitId, wait.wait);
    }
    for (const signal of persisted.signals) {
      this.#signalsById.set(signal.signalId, {
        runId: runId(signal.runId),
        signalName: signal.signalName,
        payload: signal.payload,
  ...
  #snapshotPersistedState(): PersistedProviderState {
    return {
      nextRun: this.#nextRun,
      nextClaimToken: this.#nextClaimToken,
      nextActivityClaimToken: this.#nextActivityClaimToken,
      nextSignalSequence: this.#nextSignalSequence,
      workflowIds: [...this.#workflowsById.entries()].map(([key, workflow]) => ({
        key,
        runId: String(workflow.runId)
      })),
      workflows: [...this.#workflowsByRun.values()].map((workflow) => ({
        namespace: workflow.namespace,
        workflowId: workflow.workflowId,
        workflowType: workflow.workflowType,
        taskQueue: workflow.taskQueue,
      ...
      activities: [...this.#activitiesById.entries()].map(([activityId, activity]) => ({
        activityId,
        namespace: activity.namespace,
        runId: String(activity.workflow.runId),
        task: activity.task,
        claim: activity.claim,
        terminalEventId: activity.terminalEventId
      })),
      activityMaps: [...this.#activityMapsByCommand.entries()].map(([key, map]) => ({
        commandKey: key,
        namespace: map.namespace,
        runId: String(map.workflow.runId),
        task: map.task,
        inputs: map.inputs,
        results: map.results,
      ...
      childWorkflowMaps: [...this.#childWorkflowMapsByCommand.entries()].map(([key, map]) => ({
        commandKey: key,
        namespace: map.namespace,
        runId: String(map.workflow.runId),
        task: map.task,
      ...
      waits: [...this.#waitsById.entries()].map(([waitId, wait]) => ({ waitId, wait })),
      signals: [...this.#signalsById.entries()].map(([signalId, signal]) => ({
        signalId,
        runId: String(signal.runId),
        signalName: signal.signalName,
        payload: signal.payload,
        receivedSequence: signal.receivedSequence,
        consumed: signal.consumed
...
function emptyPersistedProviderState(): PersistedProviderState {
  return {
    nextRun: 1,
    nextClaimToken: 1,
    nextActivityClaimToken: 1,
