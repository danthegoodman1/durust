...
export class SqliteBackend implements DurableBackend {
  ...
  async failActivity(req: FailActivityRequest): Promise<FailActivityOutcome> {
    return this.#transaction(() => {
      ...
      // would leave the engine's `ScheduleItemRetry` with no producer, and
      // would reschedule an item onto a map that had already ended.
      if (activity.task.mapItem !== null) {
   ...
   * no arm decides anything the engine already decided. The descriptor row is
   * rewritten once at the end rather than per effect, so one transition costs
    ...
    for (const effect of effects) {
      switch (effect.kind) {
        ...
        case "ScheduleItemRetry": {
          const activity = this.#activityForIdOrNull(
  ...
  /**
   * `AbandonPendingItems`: tombstone every not-yet-terminal item row of a map
  ...
  #abandonPendingActivityMapItems(map: ActivityMapState): void {
    const tail = tailEventId(this.#stateForRun(map.runId));
    const rows = this.#db.prepare(
      "select * from activities where map_command_key = ? and terminal_event_id is null"
    ).all(commandKey(map.task.mapCommandId)) as unknown as ActivityRow[];
    for (const row of rows) {
      const activity = activityStateFromRow(row);
      activity.terminalEventId = tail;
      activity.claim = null;
      this.#insertActivity(activity);
    }
  }
  ...
  #replaceActivityMapItemRows(map: ActivityMapState): void {
    ...
    for (let ordinal = 0; ordinal < map.inputs.length; ordinal += 1) {
      ...
      insert.run(
        ...
        mapItemInFlight(map.nextOrdinal, map.terminal, ordinal, result) ? 1 : 0,
  ...
  #completeActivityMapItem(activity: ActivityState, result: PayloadRef): EventId {
    ...
    this.#insertActivity(activity);
    return (
      this.#applyActivityMapEffects(map, transition.effects) ??
      tailEventId(this.#stateForRun(map.runId))
    );
  }
  ...
  #failActivityMapItem(
    ...
    if (map.terminal) {
      ...
      activity.claim = null;
      this.#insertActivity(activity);
      return { kind: "Failed", eventId: activity.terminalEventId };
    }
    const ordinal = activity.task.mapItem.itemOrdinal;
    ...
    if (decision.kind === "Retry") {
      ...
      return {
        ...
        readyAtMs: retried?.availableAtMs ?? 0
      };
    }
  ...
  #replaceChildWorkflowMapItemRows(map: ChildWorkflowMapState): void {
    ...
    for (let ordinal = 0; ordinal < map.inputs.length; ordinal += 1) {
      ...
      insert.run(
        ...
        mapItemInFlight(map.nextOrdinal, map.terminal, ordinal, outcome) ? 1 : 0,
 ...
 * count.
 */
function parseInFlight(json: string): number {
  const value = parseJson<number | readonly number[]>(json);
