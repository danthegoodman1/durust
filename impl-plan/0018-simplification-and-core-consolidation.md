...
- 2D.1 Make the system SQLite dependency optional for TypeScript users. Build
  memory/Postgres and SQLite addons from the same binding with Cargo features,
  package both in each existing platform package, and load the SQLite addon
  only for `NativeBackend.sqlite()`. Install `sqlite-devel` in the Linux release
  images. Check actual artifact dependencies, coexistence of both addons, and
  memory/Postgres operation without SQLite installed.
- 2D.2 Keep release artifact downloads outside the source checkout so Cargo's
  publication cleanliness check remains valid. Verify that downloaded and
  assembled native files stay out of the Rust package.
...
API budget for 2D.1: the provider constructors and backend contract stay the
same. A single addon cannot defer its linked libraries until a constructor
runs, so existing constructor composition cannot isolate SQLite. Two feature
builds reuse the binding and bound loading to at most two cached modules;
there are no new npm packages or provider implementations. The SQLite addon
uses `DURUST_SQLITE_LIBRARY_PATH` for the same local override supported by
`DURUST_NATIVE_LIBRARY_PATH` for the default addon. Shared conformance and
cross-provider lifecycle tests protect behavior; packaging tests and dependency
inspection prove the isolation. Calls keep the existing single native hop,
with no change to scheduling, replay, or durable storage.
...
| Complete | Gate | 2D.1: optional system SQLite dependency | Release `35159960289` failed both Linux links with missing `-lsqlite3`; publish was skipped. Both feature builds pass in manylinux 2.34 x64 with `sqlite-devel`. `check-native-linkage.sh` rejects the old addon and proves only the companion needs SQLite. SQLite creates/reopens storage; after removing the library in a disposable container, public TypeScript memory/Postgres constructors still work, including after a failed SQLite load. Full TypeScript check on those artifacts: 550 passed, 1 optional S3 skip; the final 5 loading/package regressions and test typecheck pass, including a reproduced/fixed custom-path override regression. System SQLite passes 26 Rust conformance tests; both feature builds and the workspace pass Clippy. Dedicated final review has no findings. Release `35164183410` passed all four platform builds, linkage checks, and addon smoke tests. |
| Complete | Gate | 2D.2: clean release staging | Release `35164183410` published `durust-macros` 0.2.2, then refused to publish `durust` because eight downloaded `.node` files were untracked inside the checkout. `release-scripts.test.ts` reproduces the exact eight-file Cargo error with the previous workflow path. Artifact downloads now use `runner.temp`; npm assembly destinations remain ignored. All 7 release-script tests and test typechecking pass. A clean copy of the actual 0.2.2 crate passes `cargo publish --dry-run --locked -p durust`, including verification compilation; no upload performed. Dedicated review has no findings. |
