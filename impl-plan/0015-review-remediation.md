...
| Complete | Doc | 8F: README benchmark documentation and baseline decision | Root README now records current three-run median mixed-workload and Criterion benchmark results with reproduction commands; TypeScript README records the matching accepted-profile medians. Checked-in baselines were left unchanged: Rust 4-worker SQLite and both Postgres workload profiles had >5% spread, and the TypeScript Postgres accepted median was 8.9% below its recorded baseline while still passing the loose threshold gate. |
