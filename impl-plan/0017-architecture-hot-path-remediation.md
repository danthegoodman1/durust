...
- TypeScript re-entrancy — the same latent bug without the UB.
  ...
  outcome to Rust. Fix: reuse the existing `#allowNondeterministicGlobalsDepth`
  counter (`:1758`/`:1762`) as a re-entrancy flag that makes durable APIs throw.
...
keeps serving other runs; a conflicted or evicted TS hot execution settles
deterministically; Miri reports no aliasing violation for the Rust nested-call
test.
...
| --- | --- | --- | --- |
| Incomplete | Work | 1A: Rust `with_context` re-entrancy guard | Missing: pointer take/restore at `src/runtime.rs:42` plus a unit test proving a nested call fails instead of aliasing. |
| Incomplete | Work | 1B: Rust `with_activity_context` guard | Missing: same guard at `src/runtime.rs:88` and its unit test. |
| Incomplete | Work | 1C: TS `resolveSideEffect` re-entrancy guard | Missing: durable APIs throwing while `#allowNondeterministicGlobalsDepth > 0` (`runtime.ts:1758`), plus its test. |
| Incomplete | Work | 1D: Rust workflow poll panic isolation | Missing: `catch_unwind` around `poll_cached` (`src/worker.rs:1646`), panic-to-`DurableFailure` mapping, cache drop. |
| Incomplete | Work | 1E: Rust activity poll panic isolation | Missing: `catch_unwind` in `start_claimed_activity` (`src/worker.rs:991`) mapping to `ActivityFinish::Fail`. |
...
| Incomplete | Test | Rust nested-durable-API replay regression | Missing: `tests/replay_core.rs` case with a mutation check that reverting 1A restores the out-of-order marker pair. |
| Incomplete | Test | TS nested-durable-API replay regression | Missing: `packages/core/test/runtime.test.ts` case with the same mutation check against 1C. |
| Incomplete | Test | Rust panicking workflow and activity regressions | Missing: `tests/worker_run.rs` cases proving the worker survives. |
| Incomplete | Test | TS abandoned-execution settlement | Missing: conflict and eviction tests asserting waiters reject. |
| Incomplete | Test | Miri aliasing check | Missing: `cargo +nightly miri test -p durust runtime::tests` result, or a recorded decision that Miri is not runnable here. |
| Incomplete | Doc | SPEC re-entrancy and fault contract | Missing: `SPEC.md` §16/§17.2 text covering both runtimes. |
...
| Incomplete | Work | 2E: Rust per-stage error isolation and exponential idle/error backoff | Missing: independent error budgets replacing `src/worker.rs:34`/`:414`. |
| Incomplete | Work | 2F: Rust worker metrics and event sink | Missing: surface matching TS `metrics()`/`onEvent`/`onError`, plus a test asserting repeated `Nondeterminism` retries are observable. |
