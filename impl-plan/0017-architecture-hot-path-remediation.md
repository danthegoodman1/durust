...
| Status | Type | Item | Evidence / Gap |
| --- | --- | --- | --- |
| Partial | Work | 5A: guard install becomes an explicit worker option | Runtime half done; default on outside production, off in production, re-read per execution. **Blocked on plumbing:** `WorkerOptions.nondeterminismGuards` did not exist, so the `Worker` — the only real production entry point — could not set it, and `NODE_ENV=production npm test` failed `worker.test.ts:3213` for exactly that reason. The reviewer notes the `NODE_ENV` re-read is **one-way**: it can turn guards on later, never off, and `nondeterminismGuards: false` does not disable guards already installed — the option is really `installIfNotAlready`, and the JSDoc over-promises. |
...
| Complete | Work | 5C: stop proxying `process.env` | `process.env` is now an accessor returning Node's real object (`types.isProxy(process.env) === false`). Measured cost of the Proxy it replaces: **+192/+186/+195 ns per read across three independent access shapes**, reproduced by the reviewer serialized with rotated arm order. `Object.keys(process.env)` — what every config library does — was **7.5×**, unreported by the implementer. |
| Partial | Work | 5D: uninstall path | `uninstallNondeterminismGuards()` written, correct, and verified over four full install/uninstall cycles plus a descriptor round-trip. **Unreachable:** not exported from `packages/core/src/index.ts`, so "embedding hosts can restore originals" is false for every consumer until the export lands. |
...
| Incomplete | Risk | `isProductionHost()` can be permanently poisoned by a stale `originalProcessEnv` | **Bug introduced by this phase.** `isProductionHost()` (`runtime.ts:471`) reads `originalProcessEnv ?? process.env`; that capture is never cleared by uninstall but *is* written by the guarded `process.env` setter (`:635`) without touching the ledger. Probe: dev → install → `process.env = {…NODE_ENV:"production"}` → uninstall leaves the real env correctly restored at `development` while the capture still says production, so **every later execution decides "production" and never installs guards again for the life of the process**. Test harnesses and config bootstraps do replace `process.env` wholesale. |
...
| Incomplete | Risk | With guards on, `console.log` of anything but a plain string throws inside workflow code | Node's `formatWithOptions`/`inspect` reads `process.env` for colour support, so the accessor fires from inside Node's own console: `console.log(obj)`, `console.error(err)`, and `util.inspect(obj)` all reject as nondeterminism. Pre-existing, and 5A improves it in production — but dev and test are exactly where guards default on and where people log. It also means an escaped-ALS unhandled rejection **cannot be logged**: the handler's own `console.log` throws from inside the escaped context. This is the strongest evidence for the plan's own alternative — "or drop it in favor of the lint rule". |
...
| Incomplete | Decision | `process.uptime` leaves the guarded set incoherent | The implementer dissented from the plan's framing and was right on the merits: `uptime()` is a monotonic clock, the same defect class as `performance.now()` and `process.hrtime()` — **both of which were kept**. Three monotonic clocks, two enforced. The blast-radius argument that justifies dropping `cpuUsage`/`memoryUsage`/`resourceUsage`/`chdir` does not transfer, since nothing's hot path calls `uptime()`. Missing: keep the guard, or state the exception. |
