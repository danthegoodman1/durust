...
| Incomplete | Work | Investigate TS Postgres mixed-workload delta (-8.9% vs recorded baseline) | Current median 192.75 workflows/s vs the 211.63 baseline (4.0% run variance); plausibly real cost from Phase 1/2 additions (per-claim lease columns, readyAtMs, terminal guard per commit) but the fixture was recreated fresh the same day, so conditions differ from the baseline recording. Per-action efficiency is fine (1.015 transactions/action, 7.393 statements/action). Needs: profile the commit/claim statement mix against the baseline run shape; accept or fix. |
