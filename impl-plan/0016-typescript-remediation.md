...
| Complete | Work | Investigate TS Postgres mixed-workload delta | Current HEAD reproduced below the checked-in `211.629 workflows/s` baseline (`186.883` median across `[186.883, 188.694, 179.877]`, then `174.867` median across 9 interleaved controls). Interleaved commit runs did not show a decisive Phase 1/2 cliff: `521a0f5` median `182.900`, `57a8782` median `175.118`, `55e80ac` median `172.415`, with HEAD controls ranging `159.837-184.513`. The checked-in baseline metadata is stale/misattributed: the recorded baseline commit `d439cf5` measures only `12.824 workflows/s` today and has the old high-cost shape (`~9.2` transactions/action, `28.251` statements/action), while the checked-in JSON records the later low-cost shape (`~1.0` transactions/action, `~7.39` statements/action). Statement profiling of HEAD versus `521a0f5` shows the same dominant statements (counter allocation and workflow-claim selection); the terminal guard is an in-memory predicate after the already-required lease row read, heartbeat writes are absent from this mixed workload, and `EXPLAIN (ANALYZE, BUFFERS)` confirms the `ready_at_ms` workflow-claim predicate uses the partial `workflow_runs_unclaimed_ready_idx`. No cheap safe TypeScript fix was identified; re-record the accepted Postgres baseline with corrected metadata after choosing a stable fixture window and slack. |
